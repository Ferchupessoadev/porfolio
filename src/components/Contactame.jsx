...
const Contactame = () => {
    ...
    const handlerClickSubmit = async (ev) => {
        ...
        try {
            let data = `${encodeURIComponent(name.current.value)}&surname=${encodeURIComponent(surname.current.value)}&email=${encodeURIComponent(email.current.value)}&message=${encodeURIComponent(message.current.value)}`;
            const ENDPOINT = `sendemail/?name=${data}`;
            const response = await fetch(
                `https://ferchuemail.000webhostapp.com/${ENDPOINT}`,
            );
    ...
    return (
        <>
            <section class="text-2xl p-3 md:p-0 w-full md:w-3/4 mx-10 flex flex-col gap-5">
                <h2 class="contactame-h2 font-firaCode text-extrabold text-lg sm:text-2xl md:text-3xl lg:text-4xl">
                    Contactame
                </h2>
                <div class="flex flex-col gap-5">
                    <form id="contactme" class="flex flex-col gap-5">
                        <input
                    ...
                    <div class="flex justify-center align-center gap-5 flex-col"></div>
                </div>
            </section>
        </>
    );
